...
├── devset-stt/               ← train transcripts
├── frames/                   ← train + test frames (1 fps)
├── features/                 ← extracted .npy files
├── llm_scalar_cache_v2.json  ← train LLM scores
└── predict/
    ├── testset_videolist_.csv
    ├── testset-stt/
    └── llm_scalar_cache_test.json
```
...
| Feature | Dim | Notes |
|---|---|---|
| Frame stats (OpenCV) | 10d | brightness, saturation, colorfulness, face rate, motion, shot cuts. Only `face_rate` has marginal signal (raw ρ=0.115) |
| LLM scalars (GPT-4o-mini) | 8d | Memorability dimensions per video, saved in `llm_scalar_cache_v2.json` |
...
| STT transcript semantic embeddings (CLIP) | ρ ≈ 0.00 |
| Frame stats except face_rate | ρ ≈ 0.00 |
| Adding title/description to TF-IDF corpus | hurts performance |
| STT truncation to first N words | full transcript always wins |
| Cross-target stacking | invalid for test — zero channel overlap → near-constant predictions |
...
### CV Strategy (shared by both)
**Channel-stratified GroupKFold** — greedy bin-packing by channel size, 5 folds, fully deterministic. Goldman Sachs (23% of data, 79 videos) gets its own dedicated fold.

---

### Approach 1 — TF-IDF + LSA + Rank Aggregation ✅ Best

**Method:**
- Full STT transcripts → TF-IDF (max 3000 features, ngram 1-3, sublinear_tf) → TruncatedSVD (LSA)
- Rank aggregation: for each LSA/LLM column, compute Spearman ρ with training target, filter by threshold, sum signed ranks on val fold
- Combine LSA and LLM rank signals by normalizing each to [0,1] then summing

**Best configs (tuned):**

| Target | n_comp | threshold | use_llm | CV ρ |
|---|---|---|---|---|
| `memorability_score` | 50 | 0.07 | Yes | **0.2697** |
| `brand_memorability` | 100 | 0.01 | No (LLM hurts) | **0.1801** |

**Key finding:** Rank aggregation consistently outperforms ML models (Ridge/SVR/XGB) on LSA features. ML can't learn from noisy high-dim LSA space with only 339 samples.

**Test prediction strategy:** Fit TF-IDF+SVD on full training corpus → transform test. Learn column directions (sign of ρ) from full training set → rank test videos among themselves.

---

### Approach 2 — ML Models (Ridge / SVR / XGBoost)

**Features:** LLM scalars (8d) + engagement metadata (3d) + face_rate (1d). No channel encoding — fully generalizable to unseen channels.

**Engagement metadata:** log_views, engagement_rate, log_duration

**Best CV results (tuned via grid search):**

| Target | Best model | CV ρ |
|---|---|---|
| `memorability_score` | Ridge (α=500) | 0.1271 |
| `brand_memorability` | SVR (C=0.5) | 0.08 |

**Note:** ML approach is significantly weaker than TF-IDF. Included for completeness in the paper — it represents a standard baseline.
...
| Script | Purpose |
|---|---|
| `extract_features.py` | Extracts visual (CLIP), frame stats features |
| `llm_score_cache.py` | Calls GPT-4o-mini to generate LLM scalar scores, saves to `llm_scalar_cache_v2.json` |
| `train_with_llm_score.py` | tunes Ridge/SVR/XGBoost across feature combos with channel-stratified CV |
| `approach_tfidf.py` | TF-IDF approach — train CV + test prediction |
| `approach_ml.py` | ML approach — train CV + test prediction |
